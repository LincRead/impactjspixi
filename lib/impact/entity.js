...
ig.Entity = ig.Class.extend({
	...
	addAnim: function( name, frameTime, sequence, stop ) {
		...
		if( !this.currentAnim ) {
			...
			if(!this.sprite && !ig.editor) { // Don't add when using Weltmeister.
				this.sprite = new PIXI.Sprite(this.currentAnim.tile);
				this.sprite.zIndex = this.zIndex;
				ig.game.stage.addChild(this.sprite);
				this.sprite.anchor = this.anchor;
			} else if(ig.editor) {
				// Load image to show when opening entity in Weltmeister.
				this.animSheet.loadWMImage();
			}
		}
	...
	update: function() {
		...
		if( this.currentAnim ) {
			...
			if(this.sprite) {
				if(this.anchor.x == 0.5) this.sprite.position.x = this.pos.x - this.offset.x - ig.game._rscreen.x + (this.size.x/2);
				else this.sprite.position.x = this.pos.x - this.offset.x - ig.game._rscreen.x + (this.size.x);

				if(this.anchor.y == 0.5) this.sprite.position.y = this.pos.y - this.offset.y - ig.game._rscreen.y + (this.size.y/2);	
				else this.sprite.position.y = this.pos.y - this.offset.y - ig.game._rscreen.y;

				var targetX = this.sprite.position.x - this.size.x - (this.size.x * this.anchor.x);
				var targetY = this.sprite.position.y - (this.size.y * this.anchor.y)// - this.size.y - (this.size.y * this.anchor.y);

				if(!
					(targetX > ig.system.width || targetY > ig.system.height ||
					targetX + this.size.x  < 0 || targetY + this.size.y < 0)) this.onScreen = true;
				...
				if(!this.currentAnim.flip.x) this.sprite.scale = new PIXI.Point(-this.scale, this.scale);
	...
	kill: function() {
		if(!ig.editor) ig.game.stage.removeChild( this.sprite );
    ...
    mouseCollisionScreen: function() {
        // PC.
        if(!ig.is_native_mobile) {
        	var mcol = ig.game.stage.getMousePosition();
            if(this.intersects(mcol.x + ig.game.screen.x, mcol.y + ig.game.screen.y )) {
                return true;
            } else {
                return false;
            }dw
    ...
    mouseCollision: function() {
        ...
        if(!ig.is_native_mobile) {
        	var mcol = ig.game.stage.getMousePosition();
            if(this.intersects(mcol.x, mcol.y)) {
                return true;
            } else {
                return false;
            }

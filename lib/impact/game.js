...
ig.Game = ig.Class.extend({
	...
	loadLevel: function( data ) {
		if(this.stage !== null) { // Remove all entities from last stage.
		...
		for( var i = 0; i < data.layer.length; i++ ) {
			...
			if( ld.name == 'collision' ) {
				...
				newMap.tileContainer.zIndex = newMap.distance;
		...
		for(var i = 0; i < ig.game.stage.children.length; i++) {
			if(ig.game.stage.children[i].texture)
				console.log(ig.game.stage.children[i].zIndex,(ig.game.stage.children[i].texture.baseTexture.imageUrl));
			else
				console.log(ig.game.stage.children[i].zIndex,ig.game.stage.children[i].children[0].texture.baseTexture.imageUrl);
		}
	...
	sortEntities: function() {
		...
		ig.game.stage.children.sort( this.sortBy );
	},
	...
	update: function(){
		...
		// Update tile sprites in all background maps.
		for( var i = 0; i < this.backgroundMaps.length; i++ ) {
	...
	draw: function(){
		...
		// Render Pixi stage.
		if(ig.game.stage)
			ig.system.renderer.render(ig.game.stage);

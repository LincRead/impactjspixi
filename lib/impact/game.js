...
ig.Game = ig.Class.extend({
	...
	loadLevel: function( data ) {
		if(this.stage !== null) { // Remove all entities from last stage.
	    	...
	    	this.stage = new PIXI.Stage(0xFFFFFF); // New stage for every level.
		}
	...
	update: function(){
		...
		for( var i = 0; i < this.backgroundMaps.length; i++ ) {
			var tiles = this.backgroundMaps[i].tileContainer.children;
			for(var j = 0; j < tiles.length; j++) {
				tiles[j].position.x = tiles[j].pos.x - ig.game._rscreen.x; // Relative x.
				tiles[j].position.y = tiles[j].pos.y - ig.game._rscreen.y; // Relative x.
			}
		}

...
ig.BackgroundMap = ig.Map.extend({	
	...
	update: function() {
		...
		for(var j = 0; j < tiles.length; j++) {
			tiles[j].position.x = tiles[j].pos.x + this.scroll.x; //ig.game._rscreen.x; // Relative x.
			tiles[j].position.y = tiles[j].pos.y + this.scroll.y; //ig.game._rscreen.y; // Relative x.
		}
	},

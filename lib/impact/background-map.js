...
ig.BackgroundMap = ig.Map.extend({	
	...
	update: function() {
		var posx = this.scroll.x;
		var posy = this.scroll.y;

		this.tileContainer.position.x = Math.floor(posx);
		this.tileContainer.position.y = Math.floor(posy);
	},
	...
	createTiles: function() {
		...
		var tile = 0,
			...
			pxMaxX = /*ig.system.width + this.tilesize*/ (this.width * this.tilesize) - pxOffsetX,
			pxMaxY = /*ig.system.height + this.tilesize*/ (this.height * this.tilesize) - pxOffsetY;
		...
		for( var mapY = -1, pxY = pxMinY; pxY < pxMaxY; mapY++, pxY += this.tilesize) {
			...
			for( var mapX = -1, pxX = pxMinX; pxX < pxMaxX; mapX++, pxX += this.tilesize ) {
				...
				if( (tile = this.data[tileY][tileX]) ) {
					...
					sprite.position.x = sprite.pos.x - ig.game._rscreen.x; // Relative x.
					sprite.position.y = sprite.pos.y - ig.game._rscreen.y; // Relative y.
